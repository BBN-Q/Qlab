...
% 04/14/2010
    ...
    properties (Access = public)
        ...
        averager;  %Configures the averager
	           ...
	           %record_length  - Number of data samples per waveform segment. 
	              ...
	              %before going to next segment. Values 1 - 65535
	           ...
	           %nbrRoundRobins - Number of times  to perform the full 
	              %segment cycle during data accumulation. - should add?

        timeout;   %Timeout for acquisition in milliseconds 
	...
	%%destructor the device
        ...
        function status = config_gen_parameter(obj, channel, parameterString, value)
            status = AqD1_configAvgConfigInt32(obj.instrID, channel, parameterString, value);
            if (status ~= 0)
        ...
        %fails to acquire
        function status = wait_for_acquisition(obj)
            %Wait for end of acquisition
            status = AqD1_waitForEndOfAcquisition(obj.instrID, obj.timeout);
            if (status ~= 0)
                disp(sprintf('Error in AqD1_waitForEndOfAcquisition: %d', status));
            end
            %unclear whether I need the rest
            if (status ~= 0)
	 ...
	 %%get data acquired
         function [dataDesc segDescArray AqDataBuffer] = transfer_waveform(obj)
            AqReadParameters.dataType = 3; % 64 bit data
            % SC: readMode = 2 for averaged waveform -  5 is for short averager waveform
            AqReadParameters.readMode = 2; % Single segment read mode
            ...
            [status dataDesc segDescArray AqDataBuffer] = AqD1_readData(obj.instrID, obj.channel_on, AqReadParameters);
            if (status ~= 0)
    ...
    methods % Instrument parameter accessors
       ...
       function val = get.vertical(obj)
           ...
           [status vert_scale offset vert_coupling bandwidth]= AqD1_getVertical(obj.instrID, obj.vertical.vert_channel);
            if (status ~= 0)
...
%                 val.sourcePattern = sourcePattern;
	...
	% no parameters really stored in the object
        ...
        function obj = set.acquire_mode(obj, modeP)
            if (isfield(modeP, 'value') && isfield(modeP, 'flags'))
	...
	%%not really useful for the averager mode - left it in anyway
        ...
        function obj = set.triggerSource(obj, trigSrcVal)
            if (isfield(trigSrcVal, 'trigger_coupling')&& isfield(trigSrcVal, 'trigger_slope') && isfield(trigSrcVal, 'trigger_level') &&  isfield(trigSrcVal, 'trigger_level2'))
        ...
        function obj = set.averager(obj, avgVal)
            if(isfield(avgVal, 'record_length') && isfield(avgVal, 'nbrSegments') && isfield(avgVal, 'num_avg') && isfield(avgVal, 'ditherRange') && isfield(avgVal, 'trigResync') && isfield(avgVal, 'data_start') && isfield(avgVal, 'data_stop'))
                %Metafunction which configures all of the averaging parameters
                status = AqD1_configAvgConfigInt32(obj.instrID, obj.channel_on, 'NbrSamples', avgVal.record_length);
                if (status ~= 0)
                    disp(sprintf('Error in AqD1_configAvgConfigInt32: %d', status));
                else
                ...
                % SC: configure the number of segments
                status = AqD1_configAvgConfigInt32(obj.instrID, obj.channel_on, 'NbrSegments', avgVal.nbrSegments);
                if (status ~= 0)
                    disp(sprintf('Error in AqD1_configAvgConfigInt32: %d', status));
                end

                status = AqD1_configAvgConfigInt32(obj.instrID, obj.channel_on, 'NbrWaveforms', avgVal.num_avg);
                if (status ~= 0)
                    disp(sprintf('Error in AqD1_configAvgConfigInt32: %d', status));
                end

                status = AqD1_configAvgConfigInt32(obj.instrID, obj.channel_on, 'DitherRange', avgVal.ditherRange);
                if (status ~= 0)
                    disp(sprintf('Error in AqD1_configAvgConfigInt32: %d', status));
                end

                status = AqD1_configAvgConfigInt32(obj.instrID, obj.channel_on, 'TrigResync', avgVal.trigResync);
                if (status ~= 0)
                    disp(sprintf('Error in AqD1_configAvgConfigInt32: %d', status));
                end

                status = AqD1_configAvgConfigInt32(obj.instrID, obj.channel_on, 'StartDelay', avgVal.data_start);
                if (status ~= 0)
                    disp(sprintf('Error in AqD1_configAvgConfigInt32: %d', status));
                end

                status = AqD1_configAvgConfigInt32(obj.instrID, obj.channel_on, 'StopDelay', avgVal.data_stop);
                if (status ~= 0)
                    disp(sprintf('Error in AqD1_configAvgConfigInt32: %d', status));
                end
            else

/**********************************************
* Module Name : libdacii.c
*
* Author/Date : B.C. Donovan / 21-Oct-08
*
* Description : Dacii functions for libdacii.dll
*
...
* Fixed a bug in libdacii.c:setupPLLFreq that was causing outputs to be disabled.
*
...
FT_HANDLE usb_handles[MAX_DACII_DEVICES];

char *cbl_dac_devices[] = CBL_SERIAL_NUMS;
#define MAX_CBL_DEVICES sizeof(cbl_dac_devices) / sizeof(char *)
 ...
 ********************************************************************/
{
	/* Initialize function points to ftd2xx library  */
	...
	#ifdef DEBUG
		freopen("libdacii.log","w", stderr);
	#endif

	#ifdef WIN32
		hdll = LoadLibrary("ftd2xx.dll");
		if ((int)hdll <= HINSTANCE_ERROR) {
			fprintf(stderr,"Error opening ftd2xx.dll library\n");
			fflush(stderr);
			return -1;
		}
		...
		if (hdll == 0) {
			fprintf (stderr, "Error opening ftd2 library: %s\n",dlerror());
			fflush(stderr);
			return -1;
		}
	#endif
...

{
	...
	if (status == FT_OK) {
	#ifdef DEBUG
		fprintf(stderr,"FTD2 Open\n");
		fflush(stderr);
	#endif
		return 0;
	} else {
	#ifdef DEBUG
		fprintf(stderr,"FTD_Open returned: %i\n", status);
		fflush(stderr);
	#endif
		return -status;
	}
...
EXPORT int DACII_ListCBLSerials()
{
	...
	max_serials = MAX_CBL_DEVICES;

	printf("This Library knows the following serial numbers\n");
	for (cnt = 0; cnt < max_serials; cnt++) {
		printf("ID: %i Serial: %s\n", cnt, cbl_dac_devices[cnt]);
	}
	return 0;
}

EXPORT int DACII_GetCBLSerials(char * buf, int maxlen, int device)
{
	if (device > MAX_CBL_DEVICES) {
		return -1;
	}
	snprintf(buf,maxlen,"%s",cbl_dac_devices[device] );
	return 0;
}
...
/******************************************************************
 *
 * Function Name : DACII_OpenByCBLID()
 *
 * Description : Opens FT2D devices
 *
 * Inputs : device - integer ID number starting at 0
...

{
	...
	serial = cbl_dac_devices[device];
	numdevices = DACII_NumDevices();
	...
	if (!found) {
		fprintf(stderr, "Could not locate device ID: %i Serial %s\n",device, serial );
		fflush(stderr);
		printf("%i\n",status);
		return -1;
	}

	fprintf(stderr,"Found device ID: %i Serial %s @ Index %i\n",device, serial,cnt );
	fflush(stderr);

	status = DACII_Open(cnt);
	if (status == FT_OK) {
		return cnt;
	} else {
		return status;
...

{
	...
	if (!found) {
		fprintf(stderr, "Could not locate device ID: Serial %s\n", serialNum );
		fflush(stderr);
		printf("%i\n",status);
		return -1;
	}

	fprintf(stderr,"Found device ID: Serial %s @ Index %i\n", serialNum,cnt );
	fflush(stderr);
...
/********************************************************************
 *
 * Function Name : DACII_NumDevices()
 *
 ...
 ********************************************************************/
{
	...
	fprintf(stderr,"Closing: %i Handle %x\n", device, usb_handles[device]);
...
/********************************************************************
*
* Function Name : DACII_ReadReg()
*
* Description : Read data from the register specified by the Command.
*                         The number of bytes depends on the Size parameter
*                         The values for the Command parameter are in dacii.h
*
* Inputs :
*               Command - DACII_FPGA_IO, DACII_FPGA_ADDR, DACII_CONF_DATA, DACII_CONF_STAT, or DACII_STATUS_CTRL
...
********************************************************************/
{
  ...
  switch(Command)
  {
    case DACII_FPGA_IO:
      Length = 1<<Size;
      break;
    case DACII_FPGA_ADDR:
    case DACII_CONF_STAT:
    case DACII_STATUS_CTRL:
      Length = 1;
      break;
    default:
  ...
  // Start all packets with a DACII Command Byte with the R/W = 1 for read
  ...
  if(usb_handle)
  {
    ...
    DLL_FT_Read(usb_handle, Data, Length, &BytesRead);
  }
  else
    return(0);
...
********************************************************************/
{
  ...
  // Start all packets with a DACII Command Byte with the R/W = 0 for write
...
*
* Inputs :
*                Command -  DACII_DAC_SPI, DACII_PLL_SPI, or DACII_VCXO_SPI
...
/********************************************************************
*
* Function Name : DACII_ProgramFpga
*
* Description :
*
* Inputs :
...
********************************************************************/
  ...
  #ifdef DEBUG
  fprintf(stderr, "Starting ProgramFPGA Device: %i Sel: %i ... \n", device, Sel);
  fflush(stderr);
  #endif


  // Create active low mask bits for PROGRAMN field
  PgmMask = DACII_PGM_BITS;
  if(Sel & 1) PgmMask &= ~DACII_PGM01_BIT;
  if(Sel & 2) PgmMask &= ~DACII_PGM23_BIT;

  // Create mask bits for valid INITN readback bits
  InitMask = 0;
  if(Sel & 1) InitMask |= DACII_INIT01_BIT;
  if(Sel & 2) InitMask |= DACII_INIT23_BIT;

  // Create active high mask bits for DONE field
  DoneMask = 0;
  if(Sel & 1) DoneMask |= DACII_DONE01_BIT;
  if(Sel & 2) DoneMask |= DACII_DONE23_BIT;

  // Read the Status to get state of RESETN for unused channel
  if(DACII_ReadReg(device, DACII_CONF_STAT, 1, 0, &ReadByte) != 1)
    return(-1);

  // Create active high mask bits for RESETN field
  // Force state of programmed chip reset to 1, leave the other alone
  RstMask = DACII_FRST_BITS & ReadByte;
  if(Sel & 1) RstMask |= DACII_FRST01_BIT;
  if(Sel & 2) RstMask |= DACII_FRST23_BIT;

  //WriteByte = DACII_OSCEN_BIT;
  //if(DACII_WriteReg(device, DACII_STATUS_CTRL, 1, 0, &WriteByte) != 1)
  // return(-5);

  // Assert PROGRAMN and deassert RESETN for the selected FPGA(s).  Leave all other bits low.
  WriteByte = PgmMask | RstMask;
  if(DACII_WriteReg(device,  DACII_CONF_STAT, 1, 0, &WriteByte) != 1)
    return(-2);
  ...
  for(i = 0; i < ByteCount; i += BLOCKSIZE)
    ...
    if(i + BLOCKSIZE < ByteCount)
    {
	  ...
	  fprintf(test,"\n");
	  #endif
      if(DACII_WriteReg(device, DACII_CONF_DATA, 0, Sel, Data+i) != BLOCKSIZE)  // Defaults to 61 bytes for CONF_DATA
        return(-8);
    }
    else
    {
      ...
      if(DACII_WriteReg(device, DACII_CONF_DATA, 0, Sel, LastBuf) != 61)  // Defaults to 61 bytes for CONF_DATA
        return(-9);
      ...
      DACII_ReadBitFileVersion(device);
...
EXPORT int DACII_SetupPLL(int device)
{
  ...
  int i;
  fprintf(stderr,"Setting PLL\n");
  fflush(stderr);

  for(i = 0; i <  sizeof(PllSetup)/sizeof(DACII_SPI_REC); i++) {
     DACII_WriteSPI(device, DACII_PLL_SPI, PllSetup[i].Address, &PllSetup[i].Data);
  }
  return 0;
}


DACII_SPI_REC PllFinish[] =
{
  0x18, 0x71,  // Initiate Calibration.  Must be followed by Update Registers Command
  0x232, 0x1,   // Set bit 0 to 1 to simultaneously update all registers with pending writes.
...
EXPORT int DACII_SetPllFreq(int device, int dac, int freq)
{
  ...
  fprintf(stderr, "Setting PLL DAC: %i Freq: %i\n", dac, freq);
  fflush(stderr);
  ...
  fprintf(stderr, "Setting PLL cycles addr: 0x%x val: 0x%x\n", pll_cycles_addr, pll_cycles_val);
  fprintf(stderr, "Setting PLL bypass addr: 0x%x val: 0x%x\n", pll_bypass_addr, pll_bypass_val);
  fflush(stderr);
  ...
  DACII_WriteSPI(device, DACII_PLL_SPI, pll_cycles_addr, &pll_cycles_val);
  DACII_WriteSPI(device, DACII_PLL_SPI, pll_bypass_addr, &pll_bypass_val);

  int i;
  for(i = 0; i <  sizeof(PllFinish)/sizeof(DACII_SPI_REC); i++) {
     DACII_WriteSPI(device, DACII_PLL_SPI, PllFinish[i].Address, &PllFinish[i].Data);
  }
 ...
 /* // Test for PLL Lock
  ...
  DACII_ReadReg(device, DACII_STATUS_CTRL, 1, 0, &ReadByte);
  if (ReadByte && DACII_LOCK_BIT != DACII_LOCK_BIT) {
...
*/
  ...
  while (test_cnt < MAX_PHASE_TEST_CNT) {
	  ...
	  for(cnt = 0; cnt < 10; cnt++) {
		xor_flag_cnt += DACII_ReadFPGA(gRegRead | FPGA_OFF_VERSION, fpga);
	  }
	  ...
	  if (xor_flag_cnt > 5) {
		...
		DACII_WriteSPI(DACII_PLL_SPI, pll_enable_addr, &pll_output_enable);
...
EXPORT int DACII_SetupVCXO(int device)
{
  ...
  fprintf(stderr, "Setting up VCX0\n");
  fflush(stderr);

  DACII_WriteSPI(device, DACII_VCXO_SPI, 0, Reg00Bytes);
  DACII_WriteSPI(device, DACII_VCXO_SPI, 0, Reg01Bytes);
  return 0;
}

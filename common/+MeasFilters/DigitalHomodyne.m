classdef DigitalHomodyne < MeasFilter
    ...
    methods
        ...
        function out = apply(obj, data)
            ...
            %frequency.
            [b,a] = my_butter(obj.IFfreq/(obj.samplingRate/2));
            
            %The signal is a 2D array with acquisition along a column
            
            %Create the weighted reference signal
            refSignal = (2/obj.integrationPts)*exp(1i*2*pi*obj.IFfreq*1/obj.samplingRate*(1:1:size(data,1)))';
            
            demodSignal = filter(b,a, ch1Signal.*repmat(refSignal,[1,size(data,2)]));

            %Box car filter the demodulated signal (as a column vector, for
            %no good reason)
            out = sum(demodSignal(obj.integrationStart:obj.integrationStart+obj.integrationPts-1,:))';
        end

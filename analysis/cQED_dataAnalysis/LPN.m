...
%data, ndataqubits, codeID, LPNq, maxqueries, nrandom
    ...
    for jj=1:nrandom
        ...
        if LPNq==1
            ...
            if majvote == str2num(reshape(xmat(codeID,:)',[],1))'; 
                successvec((nvotes-1)/querystep+1)=successvec((nvotes-1)/querystep+1)+1;
            end
        else
            for t=1:size(xmat,1)
...
figure(202); 
hold on;
plot(1:querystep:maxqueries,successvec); ylim([0,1]); 

...
if n==3: #currently largest LPN exp.
	...
	if seqkey == '111':
	    ...
	    seq = [Id(CRd3), Y90(a)*Y90(d3)] + seqCRd2 + [Z90m(a)]+ seqCRd1 + [Z90m(a), Y90(a)*X90(d1)*X90(d2),X(a)*Y90m(d1)*Y90m(d2)] + Hadgates + [MEAS(a)*MEAS(d1)*MEAS(d2)*MEAS(d3)] 
	elif seqkey == '101':
		...
		seq = [Id(CRd1), Y90(a)*Y90(d1)] + seqCRd2 + [Z90m(a)]+ seqCRd3 + [Z90m(a)] + [Y90(a)*X90(d2)*X90(d3),X(a)*Y90m(d2)*Y90m(d3)] + Hadgates + [MEAS(a)*MEAS(d1)*MEAS(d2)*MEAS(d3)] 
	elif seqkey == '010':
		seq = [Id(CRd1)*Id(CRd3), Y90(a)*Y90(d1)*Y90(d3)] + seqCRd2 + [Z90m(a)] + [Y90(a)*X90(d2),X(a)*Y90m(d2)] + Hadgates + [MEAS(a)*MEAS(d1)*MEAS(d2)*MEAS(d3)] #the Id on CRd1 is necessary to overwrite the seq. in the APS, with the current config. 
	elif seqkey == '001':
...
if n==3:
	...
	seqs+=[[MEAS(a)*MEAS(d1)*MEAS(d2)]]
	seqs+=[[X(d2), MEAS(a)*MEAS(d1)*MEAS(d2)]]
	seqs+=[[X(a), MEAS(a)*MEAS(d1)*MEAS(d2)]]
	seqs+=[[X(d1), MEAS(a)*MEAS(d1)*MEAS(d2)]]

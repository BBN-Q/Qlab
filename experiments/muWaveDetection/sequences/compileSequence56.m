function compileSequence56(basename, pg, patseq, calseq, numsteps, nbrRepeats, fixedPt, cycleLength, makePlot)
...
for n = 1:nbrPatterns;
    [I_seq{n}, Q_seq{n}, ~, PulseCollection] = pg.build(patseq{floor((n-1)/nbrRepeats)+1}, numsteps, delays('56'), fixedPt, PulseCollection);
    ...
    for stepct = 1:numsteps
        ...
        ch3m1((n-1)*stepct + stepct, :) = pg.bufferPulse(patx, paty, 0, bufferPaddings('56'), bufferResets('56'), bufferDelays('56'));
    end
end

for n = 1:calPatterns;
    [I_seq{nbrPatterns + n}, Q_seq{nbrPatterns + n}, ~, PulseCollection] = pg.build(calseq{floor((n-1)/nbrRepeats)+1}, 1, delays('56'), fixedPt, PulseCollection);
    ...
    patx = circshift(patx, delayDiff);
    paty = circshift(paty, delayDiff);
    ch3m1(nbrPatterns*numsteps + n, :) = pg.bufferPulse(patx, paty, 0, bufferPaddings('56'), bufferResets('56'), bufferDelays('56'));
end

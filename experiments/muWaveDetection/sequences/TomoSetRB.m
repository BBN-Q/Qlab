function TomoSetRB(varargin)
%Single qubit partial tomography based on randomized benchmarking 
%with Atomic Cliffords
...
basename = 'TomoRB';
fixedPt = 504*36+1000; %15000
cycleLength = fixedPt+3000; %19000
nbrRepeats = 1;
...
FID = fopen('U:\Shelby\long data\p3_char_12_overlap_5.csv');
if ~FID
...
subseqLength = length(seqStrings)/3;
fprintf('Sub sequence length: %d\n', subseqLength);
for ct = 1:3
    seqParams.suffix = num2str(ct);
    patternDict = containers.Map();
    patternDict(IQkey) = struct('pg', pg, 'patseq', {patSeqs((ct-1)*subseqLength+1:ct*subseqLength)}, 'calseq', calseq, 'channelMap', qubitMap.(qubit));
    measChannels = {'M1'};
    awgs = {'TekAWG', 'BBNAPS'};
    ...
    function outPulse = CliffPulse(cliffNum)
        ...
        defaultParams = {'pType', 'arbAxisDRAG', 'nutFreq', nutFreq, 'sampRate', pg.samplingRate, 'delta', -0.8};

...
for n = 1:nbrPatterns;
    for stepct = 1:numSteps
        ...
        ch1((n-1)*numSteps + stepct, :) = patx + ChParams.offset;
        ch2((n-1)*numSteps + stepct, :) = paty + ChParams.offset;
        ch3m1((n-1)*numSteps + stepct, :) = pg.bufferPulse(patx, paty, 0, ChParams.bufferPadding, ChParams.bufferReset, ChParams.bufferDelay);
    end
end

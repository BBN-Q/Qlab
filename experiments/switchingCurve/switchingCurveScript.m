...
cfg_file_number = 8;
cfg_file_name = 'switchingCurve_v1_008.cfg';
...
restoredefaultpath
addpath([ base_path 'expScripts/'],'-END');
addpath([ base_path 'src/matlab/'],'-END');
addpath([ base_path,'cfg'],'-END');
addpath([ base_path,'data'],'-END');
addpath([ base_path,'cfg/switchingCurve'],'-END');
addpath([ base_path,'data/switchinCurve'],'-END');
addpath([ base_path 'src/matlab/util/'],'-END');           
...
Exp = expManager.switchingCurve(base_path,cfg_file_name);
Exp.setPath;
Exp.parseExpcfgFile;
...
[Exp.inputStructure.TaskParams] = Exp.initExpParams(Exp.inputStructure.TaskParams,TimeAmpParams);
...
Exp.inputStructure.TaskParams.MicrowavePulse.time.start(1:4) = [muwave_pulse_t_begin_start muwave_pulse_riseTime_start (muwave_pulse_t_end_start-muwave_pulse_t_begin_start) muwave_pulse_riseTime_start]*1e-9;
Exp.inputStructure.TaskParams.MicrowavePulse.time.end(1:4) = [muwave_pulse_t_begin_end muwave_pulse_riseTime_end (muwave_pulse_t_end_end-muwave_pulse_t_begin_end) muwave_pulse_riseTime_end]*1e-9;
 
Exp.inputStructure.TaskParams.v1_pulse.amp.start   = [0;0;0;0;0];
Exp.inputStructure.TaskParams.v2_pulse.amp.start   = [0;0;0;0;0];
Exp.inputStructure.TaskParams.v3_pulse.amp.start   = [0;0;0;0;0];
Exp.inputStructure.TaskParams.MicrowavePulse.amp.start   = [0;0;0;0;0];
...
Exp.inputStructure.TaskParams.muwave1.frequency.start  =     4;%5.65;%5.83; % min frequency in GHz0
Exp.inputStructure.TaskParams.muwave1.frequency.end    =     8; % max frequency in GHz
Exp.inputStructure.TaskParams.muwave1.power.start      =     -19; % min power in dBm
Exp.inputStructure.TaskParams.muwave1.power.end        =     -30; % max power in dBm

% % muwave 2 is nomianlly the 02 drive
% Exp.inputStructure.TaskParams.muwave2.frequency.start =     6.4;%6.0429; % min frequency in GHz
% Exp.inputStructure.TaskParams.muwave2.frequency.end   =     6.6; % max frequency in GHz
% Exp.inputStructure.TaskParams.muwave2.power.start     =     -17;%-6.5; % min frequency in dBm
% Exp.inputStructure.TaskParams.muwave2.power.end       =     5;%-3.5; % max frequency in dBm

% % Not being used for now
% % muwave 3 can't go above -12 dBm or below -20 dBm
% Exp.inputStructure.TaskParams.muwave3.frequency.start =     5; % min frequency in GHz
% Exp.inputStructure.TaskParams.muwave3.frequency.end   =     6; % max frequency in GHz
% Exp.inputStructure.TaskParams.muwave3.power.start     =     -19; % min frequency in dBm
% Exp.inputStructure.TaskParams.muwave3.power.end       =     -10; % max frequency in dBm
% 


Exp.inputStructure.TaskParams.readoutPulse.maxAmp.start = 0.01;%1.475;%4.15;%%4.233; %in V
Exp.inputStructure.TaskParams.readoutPulse.maxAmp.end   = 0.10; %in V

% for now just remove tasks that are not necessary
Exp.inputStructure.TaskParams = rmfield(Exp.inputStructure.TaskParams,'v1_pulse');
Exp.inputStructure.TaskParams = rmfield(Exp.inputStructure.TaskParams,'v2_pulse');
Exp.inputStructure.TaskParams = rmfield(Exp.inputStructure.TaskParams,'v3_pulse');
Exp.inputStructure.TaskParams = rmfield(Exp.inputStructure.TaskParams,'MicrowavePulse');
Exp.inputStructure.TaskParams = rmfield(Exp.inputStructure.TaskParams,'muwave1');
% Exp.inputStructure.TaskParams = rmfield(Exp.inputStructure.TaskParams,'muwave2');
...
if setLoops
    ...
    LoopParams.readoutPulse.plotRange.start                  =         readoutPulse_conversionFactor*Exp.inputStructure.TaskParams.readoutPulse.maxAmp.start;
    LoopParams.readoutPulse.plotRange.end                    =         readoutPulse_conversionFactor*Exp.inputStructure.TaskParams.readoutPulse.maxAmp.end;
...
%     LoopParams.bothpowers.plotRange.start                  =         bothpowers_conversionFactor*Exp.inputStructure.TaskParams.muwave1.power.start;
%     LoopParams.bothpowers.plotRange.end                    =         bothpowers_conversionFactor*Exp.inputStructure.TaskParams.muwave1.power.end;
% % 
...
%     LoopParams.power1.plotRange.start                  =         power1_conversionFactor*Exp.inputStructure.TaskParams.muwave1.power.start;
%     LoopParams.power1.plotRange.end                    =         power1_conversionFactor*Exp.inputStructure.TaskParams.muwave1.power.end;
%     
...
%     LoopParams.power2.plotRange.start                  =         power2_conversionFactor*Exp.inputStructure.TaskParams.muwave2.power.start;
%     LoopParams.power2.plotRange.end                    =         power2_conversionFactor*Exp.inputStructure.TaskParams.muwave2.power.end;
% 
...
%     LoopParams.frequency2.plotRange.start                  =         frequency2_conversionFactor*Exp.inputStructure.TaskParams.muwave2.frequency.start;
%     LoopParams.frequency2.plotRange.end                    =         frequency2_conversionFactor*Exp.inputStructure.TaskParams.muwave2.frequency.end;
% %     
...
%     LoopParams.frequency1.plotRange.start                  =         frequency1_conversionFactor*Exp.inputStructure.TaskParams.muwave1.frequency.start;
%     LoopParams.frequency1.plotRange.end                    =         frequency1_conversionFactor*Exp.inputStructure.TaskParams.muwave1.frequency.end;
...
%     LoopParams.v2_amp.plotRange.start                  =         v2_amp_conversionFactor*Exp.inputStructure.TaskParams.v2_pulse.maxAmp.start;
%     LoopParams.v2_amp.plotRange.end                    =         v2_amp_conversionFactor*Exp.inputStructure.TaskParams.v2_pulse.maxAmp.end;
...
%     LoopParams.v3_amp.plotRange.start                  =         v3_amp_conversionFactor*Exp.inputStructure.TaskParams.v3_pulse.maxAmp.start;
%     LoopParams.v3_amp.plotRange.end                    =         v3_amp_conversionFactor*Exp.inputStructure.TaskParams.v3_pulse.maxAmp.end;
% 
...
%     LoopParams.v1_amp.plotRange.start                  =         v1_amp_conversionFactor*Exp.inputStructure.TaskParams.v1_pulse.maxAmp.start;
%     LoopParams.v1_amp.plotRange.end                    =         v1_amp_conversionFactor*Exp.inputStructure.TaskParams.v1_pulse.maxAmp.end;
%     
...
%     LoopParams.v0_offset.plotRange.start                  =         v0_offset_conversionFactor*Exp.inputStructure.TaskParams.v1_pulse.offset.start;
%     LoopParams.v0_offset.plotRange.end                    =         v0_offset_conversionFactor*Exp.inputStructure.TaskParams.v1_pulse.offset.end;
%         
...
%     %LoopParams.readoutPulse.plotRange.start     =         Exp.inputStructure.TaskParams.readoutPulse.offset.start;
%     %LoopParams.readoutPulse.plotRange.end       =         Exp.inputStructure.TaskParams.readoutPulse.offset.end;
...
%     LoopParams.pi_pulse.plotRange.start                  =         pi_pulse_conversionFactor*Exp.inputStructure.TaskParams.v2_pulse.maxAmp.start;
%     LoopParams.pi_pulse.plotRange.end                    =         pi_pulse_conversionFactor*Exp.inputStructure.TaskParams.v2_pulse.maxAmp.end;
% 
%     LoopParams.v2_amp.name                             =         'flux_loop'; %Loop name
